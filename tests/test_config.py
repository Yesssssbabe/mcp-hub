...
from mcp_hub.config import ConfigManager, MCPConfig, ClientConfig, _expand_path, _get_current_os, _get_home_dir
from mcp_hub.constants import ConfigError, SUPPORTED_CLIENTS, DEFAULT_DATA_DIR
from mcp_hub.registry import MCPTool, Registry


# Capture the real ConfigManager.get_client_config_path before any test patches it.
_orig_get_client_config_path = ConfigManager.get_client_config_path


@pytest.fixture(autouse=True)
def _patch_client_config_path(tmp_path, monkeypatch):
    """Redirect per-client config files into tmp_path during add/remove tests."""
    ...
    # _backup_file restricts backups to the home directory, so treat tmp_path as home.
...
class TestConfigManagerDetectClientConfigs:
    ...
    def test_detect_no_clients(self, tmp_path, monkeypatch):
        cm = ConfigManager(config_path=str(tmp_path / "config.json"))
        monkeypatch.setattr("mcp_hub.config.os.path.exists", lambda x: False)
    ...
    def test_detect_claude_mac(self, tmp_path, monkeypatch):
        cm = ConfigManager(config_path=str(tmp_path / "config.json"))
        monkeypatch.setattr(
            ConfigManager, "get_client_config_path", _orig_get_client_config_path
        )
        fake_path = tmp_path / "Library" / "Application Support" / "Claude" / "claude_desktop_config.json"
        fake_path.parent.mkdir(parents=True)
        fake_path.write_text("{}")
        monkeypatch.setattr("mcp_hub.config._get_current_os", lambda: "macos")
        monkeypatch.setattr("mcp_hub.config.os.path.expanduser", lambda x: str(tmp_path))
        monkeypatch.setattr("mcp_hub.config.os.path.expandvars", lambda x: str(x))
        detected = cm.detect_client_configs()
...
class TestConfigManagerValidateClientConfig:
    ...
    def test_validate_sensitive_env(self, tmp_path):
        ...
        cm = ConfigManager(config_path=str(config_path))
        cfg = ClientConfig(client_name="claude")
        cfg.add_server(MCPConfig(name="s", command="python", env={"API_KEY": "__TEST_SECRET__"}))
...
class TestConfigManagerImportExport:
    ...
    def test_import_client_config(self, tmp_path, monkeypatch):
        config_path = tmp_path / "config.json"
        cm = ConfigManager(config_path=str(config_path))
        import_path = tmp_path / "import.json"
        import_path.write_text(json.dumps({"mcpServers": {"srv": {"command": "python3"}}}))
        monkeypatch.setattr(
            "mcp_hub.config.ALLOWED_IMPORT_DIRS", [str(tmp_path), _expand_path("~/.config")]
        )
    ...
    def test_export_client_config(self, tmp_path, monkeypatch):
        config_path = tmp_path / "config.json"
        cm = ConfigManager(config_path=str(config_path))
        tool = make_tool(name="exp", mcp_server_name="exp", mcp_config_template={"command": "npx"})
        cm.add_server("claude", tool)
        monkeypatch.setattr("mcp_hub.config.DEFAULT_DATA_DIR", str(tmp_path))
        out = cm.export_client_config("claude")
...
class TestConfigManagerEdgeCases:
    ...
    def test_unicode_in_config(self, tmp_path):
        ...
        tool = make_tool(
            ...
            display_name="中文",
            description="中文",
            mcp_server_name="unicode-server",
            mcp_config_template={"command": "npx"},
        )
        cm.add_server("claude", tool)
        cm2 = ConfigManager(config_path=str(config_path))
        server = cm2.configs["claude"].servers["unicode-server"]
        assert server.description == "中文"
...
class TestBackupFile:
    """Test _backup_file helper."""

    def test_backup_file_exists(self, tmp_path, monkeypatch):
        from mcp_hub.config import _backup_file
        monkeypatch.setattr("mcp_hub.config._get_home_dir", lambda: tmp_path)
        file_path = str(tmp_path / "config.json")
        Path(file_path).write_text('{"key": "value"}')
        backup = _backup_file(file_path)
        assert backup is not None
        assert backup.startswith(file_path)
    ...
    def test_backup_file_not_exists(self, tmp_path):
        ...
        assert _backup_file(file_path) is None

    def test_backup_file_multiple(self, tmp_path, monkeypatch):
        from mcp_hub.config import _backup_file
        monkeypatch.setattr("mcp_hub.config._get_home_dir", lambda: tmp_path)
        file_path = str(tmp_path / "config.json")
        Path(file_path).write_text('{"key": "value"}')
        backup1 = _backup_file(file_path)
        backup2 = _backup_file(file_path)
        assert backup1 != backup2
        assert os.path.exists(backup1)
        assert os.path.exists(backup2)


class TestValidateClientConfigErrors:
    """Test validate_client_config error paths."""

    def test_validate_empty_server_name(self, tmp_path):
        config_path = tmp_path / "config.json"
        cm = ConfigManager(config_path=str(config_path))
        cfg = ClientConfig(client_name="claude")
        cfg.add_server(MCPConfig(name="valid", command="npx"))
        # Mutate to empty name to bypass Pydantic validator
        cfg.servers["valid"].name = ""
        cm.configs["claude"] = cfg
        errors = cm.validate_client_config("claude")
    ...
    def test_validate_empty_command(self, tmp_path):
        ...
        cm = ConfigManager(config_path=str(config_path))
        cfg = ClientConfig(client_name="claude")
        cfg.add_server(MCPConfig(name="test", command="npx"))
        cfg.servers["test"].command = ""
        cm.configs["claude"] = cfg
        errors = cm.validate_client_config("claude")
        assert any("empty command" in e.lower() for e in errors)

    def test_validate_non_serializable_global_settings(self, tmp_path):
        config_path = tmp_path / "config.json"
        cm = ConfigManager(config_path=str(config_path))
        cfg = ClientConfig(client_name="claude")
        cfg.add_server(MCPConfig(name="test", command="npx"))
        cfg.global_settings = {"bad": set()}
        cm.configs["claude"] = cfg
        errors = cm.validate_client_config("claude")
        assert any(
            "not valid" in e.lower() or "serializable" in e.lower() or "json" in e.lower()
            for e in errors
        )

    def test_validate_sensitive_env_keys(self, tmp_path):
        config_path = tmp_path / "config.json"
        cm = ConfigManager(config_path=str(config_path))
        cfg = ClientConfig(client_name="claude")
...
class TestConfigManagerConcurrency:
    """Test concurrent ConfigManager operations."""

    def test_concurrent_add_server(self, tmp_path):
        ...
        def worker(i):
            try:
                ...
                return cm.add_server("claude", tool)
            except Exception:
                return False
        ...
        cm2 = ConfigManager(config_path=str(config_path))
        names = [s.name for s in cm2.list_servers("claude")]
        assert len(names) == 10

...
class Installer:
    ...
    def _validate_tool_name(self, tool_name: str) -> None:
        ...
        Only allows Unicode word characters, hyphens, and underscores.
        """
        if not tool_name or not re.match(r'^[\w\-]+$', tool_name):
            raise ValueError(
                f"Invalid tool_name '{tool_name}': only letters, numbers, "
    ...
    def _is_valid_tool_name(self, tool_name: str) -> bool:
        ...
        return bool(re.match(r"^[\w\-]+$", tool_name))
    ...
    def uninstall(self, tool_name: str, purge: bool = False) -> InstallResult:
        ...
        try:
            ...
            return InstallResult(
                success=False,
                tool_name=tool_name,
                message=f"Uninstalled but registry update failed: {msg}",
                install_path=install_path,
                errors=[msg],
            )
        ...
        if log_warnings:
            return InstallResult(
                success=False,
                tool_name=tool_name,
                message=f"Uninstalled '{tool_name}' with warnings",
                install_path=install_path,
                errors=log_warnings,
            )
    ...
    def _run_command(
        ...
        try:
            ...
            return -1, stdout, f"Network timeout after {timeout}s — please check your internet connection and try again.\n{stderr}"

...
# ---- Runtime config base (FIX-01) ----
def _get_config_base() -> Path:
    """Return the effective configuration base directory at runtime.

    Respects MCP_HUB_CONFIG_DIR environment variable; falls back to
    DEFAULT_DATA_DIR.  Uses Path(...).resolve() to normalize macOS
    symlinks.
    """
...
DEFAULT_SAFE_BASE = _get_config_base()
TOOL_NAME_PATTERN = re.compile(r'^(?:(?:@[\w\-]+/[\w\-]+)|[\w\-]+)$')
...
def safe_makedirs(path: str, mode: int = 0o700) -> None:
    ...
    - Traverses all path components and rejects any symlink.
    - Checks W_OK|X_OK on existing directories; refuses if not writable.
    - Explicitly chmods 0o700 after creation.
    - Re-verifies after creation to catch race-condition symlink swaps (TOCTOU).
    """
    p = Path(path)
    # Traverse every component and reject symlinks
    for part in p.parents:
        if part.exists() and part.is_symlink():
            raise SecurityError(f"Path component is a symbolic link: {part}")
    if p.exists() and p.is_symlink():
        raise SecurityError(f"Path cannot be a symbolic link: {path}")

    # Check existing directories for writability/executability
    if p.exists():
        if not os.access(str(p), os.W_OK | os.X_OK):
            raise SecurityError(f"Directory is not writable/executable: {path}")
    else:
        # Check parent dirs up to the first existing one
        for parent in p.parents:
            if parent.exists():
                if not os.access(str(parent), os.W_OK | os.X_OK):
                    raise SecurityError(f"Parent directory is not writable/executable: {parent}")
                break

    os.makedirs(path, mode=mode, exist_ok=True)

    # TOCTOU: re-verify after creation
    if os.path.islink(path):
        raise SecurityError(
...
def _test_dir_writable(path: Path) -> None:
    """Verify a directory is actually writable by creating a temporary file.

    Raises MCPHubError if the directory is not writable.
    """
    test_file = path / ".mcp_hub_writable_test"
    try:
        test_file.write_text("test", encoding="utf-8")
        test_file.unlink()
    except (PermissionError, OSError) as exc:
        raise MCPHubError(f"Directory is not writable: {path} — {exc}")


# ---- CLI unified error handler (FIX-024) ----
CATEGORY_CORRUPTED = ("corrupted data", "json", "decode", "parse")
CATEGORY_PERMISSION = ("permission denied", "permission", "access denied")
CATEGORY_MISSING = ("not found", "no such file", "does not exist", "missing")


def _handle_cli_error(cmd_name: str, exc: Exception) -> None:
    ...
    if any(c in msg for c in CATEGORY_CORRUPTED):
        ...
        console.error(f"Operation failed: {exc}")
    else:
        console.error("An unexpected error occurred. Please check the logs.")
    logger.debug("Error in %s: %s", cmd_name, exc)


# ---- Output helpers (FIX-058) ----
def _output_json(data: Union[list, dict]) -> None:
    """Emit valid JSON to stdout; empty collections become [] or {}."""
    if data is None:
        data = []
...
def _mask_env_values(env: Dict[str, str]) -> Dict[str, str]:
    ...
    for key, value in env.items():
        lower_key = key.lower()
        if any(s in lower_key for s in ("api_key", "apikey", "api_secret", "apisecret", "secret", "token", "password", "auth", "credential")):
            masked[key] = "***"
        else:
            masked[key] = value
    return masked
...
def _mask_hub_config(data: Dict[str, Any]) -> Dict[str, Any]:
    """Sanitize hub config for display: mask absolute command paths and env secrets."""
    from copy import deepcopy
    safe = deepcopy(data)
    for client_name, client_cfg in safe.items():
        if not isinstance(client_cfg, dict):
            continue
        servers = client_cfg.get("servers", {})
        for server_name, server in servers.items():
            if not isinstance(server, dict):
                continue
            cmd = server.get("command", "")
            if cmd and os.path.isabs(cmd):
                server["command"] = os.path.basename(cmd)
            env = server.get("env", {})
            if env:
                server["env"] = _mask_env_values(env)
    return safe
...
def _validate_name(name: str) -> str:
    ...
    Only allows Unicode word characters, underscore, and hyphen. Length 1-64.
    """
    if not name or len(name) > MAX_NAME_LENGTH:
        raise MCPHubError(f"Name too long or empty (max {MAX_NAME_LENGTH} chars)")
    if not re.match(r'^[\w\-]{1,64}$', name):
        raise MCPHubError(f"Invalid name format: {name}. Only [\\w\\-] allowed.")
    return name
...
def _validate_tags(tags: List[str]) -> List[str]:
    ...
    for tag in tags:
        ...
        if not re.match(r'^[\w\-]{1,64}$', tag):
            raise MCPHubError(f"Invalid tag format: {tag}. Only [\\w\\-] allowed.")
        safe_tags.append(tag)
    return safe_tags


def _validate_category(category: str) -> str:
    """Validate category input: length limit and format checks to prevent ReDoS."""
    if not category or len(category) > MAX_INPUT_LENGTH:
        raise MCPHubError(f"Category too long or empty (max {MAX_INPUT_LENGTH} chars)")
    if _SHELL_METACHARS.search(category):
        raise MCPHubError(f"Category contains forbidden characters: {category}")
    return category
...
def init(
    ...
    """
    try:
        # FIX-090: detect missing HOME and warn
        if not os.environ.get("HOME") and not os.environ.get("MCP_HUB_CONFIG_DIR"):
            console.warning("HOME environment variable is not set. Using current directory as fallback.")
        ...
        # FIX-006: check existing files before init (unless --force)
        reg_exists = Path(validated_registry).exists() if validated_registry else False
        cfg_exists = Path(validated_config).exists() if validated_config else False
        if not force and (reg_exists or cfg_exists):
            if not sys.stdin.isatty():
                console.error("Registry or config already exists. Use --force to overwrite.")
                raise typer.Exit(1)
            confirm = typer.confirm("Registry or config already exists. Overwrite?")
            if not confirm:
                console.info("Init cancelled")
                raise typer.Exit(0)
        ...
        # FIX-031: preserve original timestamps when force-clearing
        original_timestamps = {}
        if force:
            for name, tool in registry.tools.items():
                original_timestamps[name] = (tool.created_at, tool.updated_at)
        ...
        with registry._lock:
            for tool in builtin_tools:
                if tool.name not in registry.tools or force:
                    ...
                    # Restore timestamps if we cleared earlier
                    if force and tool.name in original_timestamps:
                        existing = registry.tools[tool.name]
                        existing.created_at = original_timestamps[tool.name][0]
                        existing.updated_at = original_timestamps[tool.name][1]
        registry._save()
        config._save()
...
def search(
    ...
    """
    try:
        validated_registry = _resolve_and_validate(registry_path, _get_config_base())
        registry = Registry(validated_registry)
        tag_list = _validate_tags(tags.split(",")) if tags else None
        # FIX-082: validate sort_by before calling search
        valid_sort_fields = {"relevance", "stars", "name", "recent", "downloads"}
        if sort_by not in valid_sort_fields:
            console.error(f"Invalid sort_by: {sort_by}. Valid: {', '.join(valid_sort_fields)}")
            raise typer.Exit(1)
        # FIX-083: validate category
        if category:
        ...
        if json_output:
            # FIX-055: use model_dump_public to mask sensitive env vars
            tools_json = [tool.model_dump_public() for tool in results]
            _output_json(tools_json)
            return
...
def install(
    ...
    try:
        ...
        validated_registry = _resolve_and_validate(registry_path, _get_config_base())
        validated_base = _resolve_and_validate(base_dir, _get_config_base(), allow_base_dir=True)
        ...
        if dry_run:
            # FIX-062/077: respect --method in dry-run
            if method != "auto":
                if method not in INSTALL_TYPES:
                    console.error(f"Unsupported install method: {method}. Supported: {', '.join(INSTALL_TYPES)}")
                    raise typer.Exit(1)
                chosen_method = method
            else:
                chosen_method = installer._detect_install_method(tool)
            target_dir = os.path.join(
                validated_base or str(DEFAULT_DATA_DIR / "installed"), sanitized_name
            )
            console.info(f"[DRY RUN] Would install '{sanitized_name}':")
            console.info(f"  Method: {chosen_method}")
        ...
        # FIX-040/075: show progress hint with chosen method
        chosen_method = method if method != "auto" else installer._detect_install_method(tool)
        console.info(f"Connecting to remote registry via {chosen_method}... (timeout: 60s)")
            ...
            if client:
                ...
                for cli_name in client:
                    ...
                    try:
                        ...
                        # FIX-085: downgrade to debug to avoid exposing internal errors
                        logger.debug(f"Failed to configure {sanitized_name} for {cli_name}: {e}")
                if configured_clients:
                    console.info(f"Configured for clients: {', '.join(configured_clients)}")
        else:
            # FIX-041/076: detect network timeout and emit friendly error
            err_msg = result.message or ""
            if "timeout" in err_msg.lower() or "time out" in err_msg.lower() or "network timeout" in err_msg.lower():
                console.error("Network timeout — please check your internet connection and try again.")
            else:
...
def uninstall(
    ...
    try:
        ...
        # FIX-017: non-interactive detection for uninstall confirmation
        if not yes:
            if not sys.stdin.isatty():
                console.error(f"Non-interactive environment detected. Use --yes to uninstall '{sanitized_name}'.")
                raise typer.Exit(1)
            try:
...
def list_tools(
    ...
    try:
        ...
        if not tools:
            # FIX-059: differentiated empty messages
            if json_output:
                _output_json([])
                return
            # Check if registry is initialized (file exists and has content)
            reg_file = Path(registry.registry_path)
            registry_initialized = reg_file.exists() and reg_file.stat().st_size > 2
            if not registry_initialized:
                rich_console.print("[yellow]Registry not initialized.[/yellow]")
            elif installed:
                rich_console.print("[yellow]No installed tools found.[/yellow]")
            elif category:
                rich_console.print(f"[yellow]No tools found in category '{category}'.[/yellow]")
            else:
                rich_console.print("[yellow]No tools found[/yellow]")
            rich_console.print("[dim]Tip: Run 'mcp-hub init' to create a registry with built-in tools, or verify the registry path with 'mcp-hub list --registry <path>'.[/dim]")
            return
        ...
        if json_output:
            # FIX-055: use model_dump_public for safe JSON output
            tools_data = [tool.model_dump_public() for tool in tools]
            _output_json(tools_data)
        else:
...
def config(
    ...
    env: Annotated[Optional[List[str]], typer.Option("--env", "-e", help="Environment variables (KEY=VALUE, supports comma-separated format like 'KEY1=VALUE1,KEY2=VALUE2', can be used multiple times. If values contain commas, use multiple --env arguments)")] = None,
    ...
    """
    try:
        validated_config = _resolve_and_validate(config_path, _get_config_base())
        ...
        if action == "add":
            ...
            # FIX-034: Parse env vars from --env KEY=VALUE (supports comma-separated)
            env_dict: Dict[str, str] = {}
            if env:
                for env_var in env:
                    # Support comma-separated env vars in a single --env argument
                    for sub_var in env_var.split(","):
                        sub_var = sub_var.strip()
                        if not sub_var:
                            continue
                        if "=" not in sub_var:
                            console.error(f"Invalid env format: {sub_var}. Use KEY=VALUE")
                            raise typer.Exit(1)
                        key, value = sub_var.split("=", 1)
                        if not key.strip():
                            console.error("Invalid env format: empty key")
                            raise typer.Exit(1)
                        env_dict[key] = value
                _validate_env(env_dict)
            cfg = MCPConfig(name=name, command=command, args=arg_list, env=env_dict)
            # FIX-009: use add_server_config for dual-write sync
            cm.add_server_config(target_client, cfg)
            ...
            with open(cm.config_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            # FIX-036: filter by --client if specified
            if client and client in data:
                data = {client: data[client]}
            elif client and client not in data:
                rich_console.print(f"[yellow]No configuration found for client '{client}'.[/yellow]")
                return
            # FIX-056: mask sensitive fields before display
            safe_data = _mask_hub_config(data)
            console.info(json.dumps(safe_data, indent=2, ensure_ascii=False))
...
def scan(
    ...
    if quick and detailed:
        console.warning("Both --quick and --detailed specified. --detailed takes precedence.")
        quick = False
    ...
    try:
        ...
        for name in tool_names:
            ...
            # FIX-046: not_installed check before severity filter
            if getattr(report, "status", "scanned") == "not_installed":
                console.info(f"Tool '{name}' is not installed. Skipping scan.")
                continue

            # Severity filter
            if severity:
                severity_upper = severity.upper()
                # FIX-045: add CRITICAL to valid levels
                valid_levels = ["UNKNOWN", "LOW", "MEDIUM", "HIGH", "VERIFIED", "CRITICAL"]
            ...
            if quick:
                if json_output:
                    _output_json({"tool_name": name, "score": report.overall_score})
                else:
                ...
                if json_output:
                    # FIX-068: use to_dict with exclude_none=True
                    _output_json(report.to_dict(exclude_none=True))
                    continue

...
from mcp_hub.constants import DEFAULT_DATA_DIR, RegistryError

_logger = logging.getLogger(__name__)


def _get_default_registry_path() -> str:
    return str(DEFAULT_DATA_DIR / "registry.json")


def _get_allowed_registry_dir() -> Path:
...
class MCPTool(BaseModel):
    ...
    npm_package: Optional[str] = Field(None, pattern=r"^[\w.@-]+$")
    pip_package: Optional[str] = Field(None, pattern=r"^[\w.@-]+$")
    pip_editable: bool = Field(False)
    docker_image: Optional[str] = Field(None, pattern=r"^[\w.:/@-]+$")
    ...
    def model_dump_public(self) -> Dict[str, Any]:
        ...
        if isinstance(env_vars, list):
            data["env_vars"] = ["***" if v else v for v in env_vars]
        return data
...
class Registry:
    ...
    def __init__(self, registry_path: Optional[str] = None):
        if registry_path:
            ...
            if env_dir:
                ...
                raw_path = os.path.expanduser(_get_default_registry_path())
    ...
    def _sanitize_path(self, raw_path: str) -> str:
        ...
        if os.environ.get("MCP_HUB_TEST_MODE") == "1":
            return str(resolved)
        allowed_bases = [_get_allowed_registry_dir().resolve()]
        env_dir = os.environ.get("MCP_HUB_CONFIG_DIR")
        if env_dir:
        ...
        for allowed in allowed_bases:
            try:
                ...
                continue
        raise ValueError(
            "Registry path must be within the allowed directory."
        )

    def acquire_init_lock(self) -> Optional[Any]:
        """Acquire a non-blocking exclusive init lock.

        Uses fcntl on Unix, falls back to filelock on other platforms.
        Returns the lock handle or None if locking is unavailable.
        """
        lock_file = Path(self.registry_path).parent / ".init.lock"
        try:
        ...
        if _HAS_FCNTL:
            ...
            try:
                flock = FileLock(str(lock_file), timeout=0)
                flock.acquire()
                return flock
            except Exception:
                return None
    ...
    def release_init_lock(self, lock_handle: Optional[Any]) -> None:
    ...
    def _load_with_cache(self) -> None:
        """Load registry with file-based caching."""
        if not os.path.exists(self.registry_path):
            return
        try:
            stat = os.stat(self.registry_path)
            cache_key = f"{self.registry_path}:{stat.st_mtime}:{stat.st_size}:{stat.st_ino}"
        except OSError:
            self._load()
            return

        with Registry._CACHE_LOCK:
            cached = Registry._file_cache.get(cache_key)
            if cached is not None:
                with self._lock:
                    self.tools = copy.deepcopy(cached)
                return

        self._load()

        with Registry._CACHE_LOCK:
            Registry._file_cache[cache_key] = copy.deepcopy(self.tools)

    def _invalidate_cache(self) -> None:
        """Invalidate all cache entries for this registry path."""
        with Registry._CACHE_LOCK:
            keys_to_remove = [k for k in Registry._file_cache if k.startswith(f"{self.registry_path}:")]
            for k in keys_to_remove:
                del Registry._file_cache[k]

    def _read_locked(self, path: str) -> dict:
    ...
    def _load_tools(self, data: dict) -> None:
        """Load tools from parsed registry data."""
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            data = self._read_locked(self.registry_path)
            with self._lock:
                self._load_tools(data)
        except (json.JSONDecodeError, OSError, ValueError, RecursionError, TypeError, ValidationError):
            # Corruption detected — attempt backup recovery
            backup_path = self.registry_path + ".bak"
            if os.path.exists(backup_path):
                try:
                    data = self._read_locked(backup_path)
                    with self._lock:
                        self._load_tools(data)
                    _logger.info("Registry restored from backup after corruption detected.")
                    # Preserve corrupted file for inspection
                    corrupted_path = (
                        self.registry_path + ".corrupted." + datetime.now().strftime("%Y%m%d%H%M%S")
                    )
                    os.rename(self.registry_path, corrupted_path)
                    return
                except Exception:
                    _logger.error("Registry backup is also unusable.")
                    raise RegistryError(
                        "Registry file is corrupted and backup is unusable. "
                        "Run 'mcp-hub init --force' to recreate."
                    )
            else:
                raise RegistryError(
                    "Registry file is corrupted and no backup exists. "
                    "Run 'mcp-hub init --force' to recreate."
                )

    def _rotate_backups(self) -> None:
        """Rotate backup chain .bak → .bak.1 → .bak.2 → ... → .bak.5."""
        base = self.registry_path + ".bak"
        if not os.path.exists(base):
            return
        for i in range(4, 0, -1):
            src = base + (f".{i}" if i > 1 else "")
            dst = base + f".{i + 1}"
    ...
    def _save(self) -> None:
        ...
        with self._lock:
            ...
            # 2) Rotate backups before overwriting current file
            self._rotate_backups()

            # 3) Acquire exclusive lock on a separate lock file (inode-safe)
            if _HAS_FCNTL:
                lock_path = self.registry_path + ".lock"
                with open(lock_path, "w", encoding="utf-8") as lock_f:
                    fcntl.flock(lock_f.fileno(), fcntl.LOCK_EX)
                    try:
                        # 4) Backup current file before replacing
    ...
    def search(
        ...
        # Filter by query string (matches name, display_name, description, tags, long_description)
        if query_lower:
            results = [
                tool
                for tool in results
                ...
                or query_lower in tool.long_description.lower()
        ...
        # Filter by category (case-insensitive, space-tolerant)
        if category:
            cat_normalized = category.lower().replace(" ", "")
            results = [
                tool
                for tool in results
                if any(cat_normalized == cat.lower().replace(" ", "") for cat in tool.categories)
            ]
    ...
    def batch_mode(self):
        """Context manager to batch mutations and save once at exit."""
        return _BatchMode(self)
...
class _BatchMode:
    ...
    def __enter__(self):
        self.registry._batch_mode = True
...
def _builtin_tools() -> List[MCPTool]:
    ...
    for tool in tools:
        tool.created_at = _BUILTIN_TIMESTAMP
        tool.updated_at = _BUILTIN_TIMESTAMP
    return tools
